      ...
      <table className="w-full text-ui-xs">
        ...
        <tbody>{articles.map(a => (
          <tr key={a.id} className="border-b-2 border-grey-200/50 last:border-b-0">
            ...
            <td className="px-4 py-3 text-right"><div className="flex items-center justify-end gap-3"><Link href={`/write?edit=${a.nostrEventId}`} className="text-grey-400 hover:text-black">Edit</Link><button onClick={() => handleDelete(a.id)} disabled={deletingId===a.id} className="text-grey-300 hover:text-black disabled:opacity-50">{deletingId===a.id ? '...' : 'Delete'}</button></div></td>
          </tr>
        ))}</tbody>
      </table>
    </div>
  ...
  return (
    <div>
      ...
      {!stripeReady && earnings && earnings.earningsTotalPence > 0 && (
        <div className="mb-10 bg-grey-100 border-l-[3px] border-crimson px-6 py-4">
          <p className="text-ui-sm text-black">You've earned £{(earnings.earningsTotalPence/100).toFixed(2)} from {earnings.readCount} paid reads. Connect your bank to get paid.</p>
          <a href="/settings" className="mt-2 inline-block text-ui-xs text-crimson-dark underline underline-offset-4">Connect Stripe</a>
        </div>
      )}

      {/* Summary cards */}
      {loading ? (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 mb-14">{[1,2,3].map(i => <div key={i} className="bg-white p-6"><div className="h-3 w-20 animate-pulse bg-grey-100 mb-3"/><div className="h-7 w-28 animate-pulse bg-grey-100"/></div>)}</div>
      ) : earnings ? (
      ...
      {subscribers.length > 0 && (
        <div className="mb-14">
          <p className="label-ui text-grey-400 mb-4">Subscribers</p>
          <div className="overflow-x-auto bg-white">
            <table className="w-full text-ui-xs">
              <thead><tr className="border-b-2 border-grey-200/50">
                <th className="px-4 py-3 text-left label-ui text-grey-400">Reader</th>
                <th className="px-4 py-3 text-right label-ui text-grey-400">Pays</th>
                <th className="px-4 py-3 text-right label-ui text-grey-400">Articles read</th>
                <th className="px-4 py-3 text-right label-ui text-grey-400">Article value</th>
                <th className="px-4 py-3 text-center label-ui text-grey-400">Value</th>
                <th className="px-4 py-3 text-left label-ui text-grey-400">Status</th>
              </tr></thead>
              <tbody>{subscribers.map(s => (
                <tr key={s.subscriptionId} className="border-b-2 border-grey-200/50 last:border-b-0">
                  <td className="px-4 py-3">
                    <Link href={`/${s.readerUsername}`} className="text-black hover:opacity-70">{s.readerDisplayName ?? s.readerUsername}</Link>
                  </td>
                  ...
                  <td className="px-4 py-3 text-center">
                    {s.gettingMoneysworth ? (
                      <span className="text-crimson text-ui-xs font-medium" title="Reading more than they pay — getting their money's worth">Good value</span>
                    ) : (
                      <span className="text-amber-600 text-ui-xs" title="Reading less than subscription cost — may cancel">At risk</span>
                    )}
                  </td>
                  <td className="px-4 py-3">
                    <span className={`text-ui-xs ${s.status === 'active' ? 'text-crimson font-medium' : 'text-grey-300'}`}>
      ...
      <div className="bg-white p-6 mb-12">
        <p className="label-ui text-grey-400 mb-3">How credits work</p>
        <div className="space-y-2 text-ui-xs text-grey-600 leading-relaxed">
      ...
      {/* Per-article revenue */}
      <p className="label-ui text-grey-400 mb-4">Per-article revenue</p>
      {loading ? <div className="space-y-3">{[1,2,3].map(i => <div key={i} className="h-8 animate-pulse bg-white"/>)}</div>
      : articleEarnings.length === 0 ? <p className="text-ui-xs text-grey-300">No settled revenue yet.</p>
      : <div className="overflow-x-auto bg-white"><table className="w-full text-ui-xs"><thead><tr className="border-b-2 border-grey-200/50"><th className="px-4 py-3 text-left label-ui text-grey-400">Article</th><th className="px-4 py-3 text-right label-ui text-grey-400">Reads</th><th className="px-4 py-3 text-right label-ui text-grey-400">Earned</th><th className="px-4 py-3 text-right label-ui text-grey-400">Pending</th><th className="px-4 py-3 text-right label-ui text-grey-400">Paid</th></tr></thead><tbody>{articleEarnings.map(a => <tr key={a.articleId} className="border-b-2 border-grey-200/50 last:border-b-0"><td className="px-4 py-3"><a href={`/article/${a.dTag}`} className="text-black hover:opacity-70">{a.title}</a></td><td className="px-4 py-3 text-right tabular-nums">{a.readCount}</td><td className="px-4 py-3 text-right text-black tabular-nums">£{(a.netEarningsPence/100).toFixed(2)}</td><td className="px-4 py-3 text-right text-grey-300 tabular-nums">£{(a.pendingPence/100).toFixed(2)}</td><td className="px-4 py-3 text-right text-grey-300 tabular-nums">£{(a.paidPence/100).toFixed(2)}</td></tr>)}</tbody></table></div>}
    </div>
  )
}
...
function AccountsTab({ userId, freeAllowancePence, hasCard }: { userId: string; freeAllowancePence: number; hasCard: boolean }) {
  ...
  if (loading) {
    return <div className="space-y-3">{[1,2,3].map(i => <div key={i} className="h-10 animate-pulse bg-white" />)}</div>
  }
    ...
    <div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 mb-10">
        <button onClick={() => handleFilterChange('credits')} className="text-left">
          <div className={`p-6 transition-shadow ${filter === 'credits' ? 'bg-black ring-2 ring-grey-600' : 'bg-black'}`}>
            <p className="label-ui mb-2 text-grey-300">Credits</p>
            <p className="font-serif text-2xl font-light text-white">£{(creditsPence/100).toFixed(2)}</p>
            <p className="mt-1 text-ui-xs text-grey-300">All income{summary?.lastSettledAt ? ' since last settlement' : ''}</p>
          </div>
        </button>
        <button onClick={() => handleFilterChange('debits')} className="text-left">
          <div className={`p-6 bg-grey-100 transition-shadow ${filter === 'debits' ? 'ring-2 ring-accent' : ''}`}>
            <p className="label-ui mb-2 text-crimson-dark">Debits</p>
            <p className="font-serif text-2xl font-light text-crimson-dark">£{(debitsPence/100).toFixed(2)}</p>
            <p className="mt-1 text-ui-xs text-crimson">All outgoings{summary?.lastSettledAt ? ' since last settlement' : ''}</p>
          </div>
        </button>
        <button onClick={() => handleFilterChange('all')} className="text-left">
          <div className={`p-6 bg-white transition-shadow ${filter === 'all' ? 'ring-2 ring-grey-600' : ''}`}>
            <p className="label-ui mb-2 text-grey-400">Balance</p>
            <p className={`font-serif text-2xl font-light ${balancePence >= 0 ? 'text-black' : 'text-crimson'}`}>
              {balancePence < 0 ? '−' : ''}£{(Math.abs(balancePence)/100).toFixed(2)}
            </p>
            <p className="mt-1 text-ui-xs text-grey-300">
      ...
      {error && <div className="bg-white px-4 py-3 text-ui-xs text-black mb-8">{error}</div>}

      {!hasCard && freeAllowancePence <= 0 && (
        <div className="mb-8 bg-grey-100 border-l-[3px] border-crimson px-6 py-4">
          <p className="text-ui-xs text-black">Free allowance used. <a href="/settings" className="underline underline-offset-4 text-crimson-dark">Add a card</a> to keep reading.</p>
        </div>
      )}
      ...
      {entries.length > 0 ? (
        <>
          <div className="flex items-center justify-between mb-4">
            ...
            {filter !== 'all' && (
              <button onClick={() => handleFilterChange('all')} className="text-ui-xs text-grey-300 hover:text-black underline underline-offset-4">
                Show all
              </button>
            )}
          </div>
              ...
              <tbody>
                {entries.map(entry => (
                  <tr key={entry.id} className="border-b-2 border-grey-200/50 last:border-b-0">
                    <td className="px-4 py-3 text-grey-300 whitespace-nowrap">
                    ...
                    <td className={`px-4 py-3 text-right tabular-nums font-medium ${
                      entry.type === 'credit' ? 'text-black'
                      : entry.type === 'settlement' ? 'text-grey-400'
                      : 'text-crimson'
                    }`}>
        ...
        <div className="py-20 text-center">
          <p className="text-ui-sm text-grey-400 mb-4">No transactions yet.</p>
          <Link href="/feed" className="text-ui-xs text-black underline underline-offset-4">Browse the feed</Link>
        </div>
      )}
    </div>
  )
...
function SettingsTab() {
  ...
  return (
    <div className="py-20 text-center">
      <p className="text-ui-sm text-grey-400">Redirecting to settings…</p>
    </div>
  )
}
...
function Card({ label, pence, sub, primary=false, accent=false }: { label: string; pence: number; sub: string; primary?: boolean; accent?: boolean }) {
  const bg = primary ? 'bg-black' : accent ? 'bg-grey-100' : 'bg-white'
  const labelColor = primary ? 'text-grey-300' : accent ? 'text-crimson-dark' : 'text-grey-400'
  const valueColor = primary ? 'text-white' : accent ? 'text-crimson-dark' : 'text-black'
  const subColor = primary ? 'text-grey-300' : accent ? 'text-crimson' : 'text-grey-300'
